...
def plot_model(fn, refA, refB, predictedA, predictedB):
    ...
    with PdfPages(fn) as pdf:
        ...
        axs[1,0].imshow(np.average(refA,axis=0)[:,:,0],vmin=0.0,vmax=0.7)
        axs[1,0].set_title('average A sample')
        axs[1,1].imshow(np.average(predictedA,axis=0)[:,:,0],vmin=0.0,vmax=0.7)
